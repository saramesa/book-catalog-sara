...
const BadgeGenre = ({ genres, showDeleteButton, bookid, handleDeleteGenre }) => {
  return (
    genres.map(genre => (
     <div key={genre}>
        <Badge variant="secondary" className="badge-genre" key={genre}>{genre}</Badge>
        { showDeleteButton? <span><i className="far fa-times-circle fa-times-custom" id={genre} onClick={deleteGenre.bind(this, genre, bookid, handleDeleteGenre)}></i></span> : null }
      </div>
    ))
  )
}
